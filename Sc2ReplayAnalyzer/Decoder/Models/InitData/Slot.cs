...
public class RewardOverdrive(long key, List<long> rewards)
{
    public long Key { get; } = key;
    public List<long> Rewards { get; } = rewards;
}

public class Slot
{
    public Slot(int aCEnemyRace, string toonHandle, List<RewardOverdrive> rewardOverrides, int? userId, string skin, List<int> commanderMasteryTalents, int aiBuild, int teamId, List<int> rewards, int commanderLevel, int logoIndex, List<string> artifacts, int difficulty, int? tandemLeaderId, int commanderMasteryLevel, int trophyId, int brutalPlusDifficulty, int? racePref, int? tandemId, string hero, string commander, string mount, int handicap, string observe, int aCEnemyWaveType, int control, List<int> licenses, int? colorPref, bool hasSilencePenalty, int workingSetSlotId, List<int> retryMutationIndexes, int? selectedCommanderPrestige)
    {
    ...
    public List<RewardOverdrive> RewardOverrides { get; }
